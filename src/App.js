...
const columns = [
  ...
  },
  {
    ...
    render: (text) =>
      text === "in stock" ? (
        <Tag color={"green"} key={text}>{text[0].toUpperCase() + text.slice(1)}</Tag>
      ) : (
        <Tag color={"red"} key = {text}>{text[0].toUpperCase() + text.slice(1)}</Tag>
      ),
  },
];
...
function App() {
  ...
  return (
    ...
    <div className="header">
      <InputComp placeholder="Search by name" value={search}
